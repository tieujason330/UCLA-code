...
void readNProcessFile(char *fileName, int sockfd, packet& p, unsigned start_t);
...
int main(int argc, const char * argv[]) {
	...
	while(1) {
		...
		//These three statements are the updated codes.
        strncpy(buffer, p_in.data, PACKETSIZE);
		buffer[p_in.size] = 0;
		...
		readNProcessFile(buffer, sockfd, p_out, start_t);
		...
		//initialize GBN
		current = 0;        //This variable is used
		seq_base = 0;       //This is the sequence number
		...
		while(seq_base < total) {
            ...
            //Response before time-out (There is a message from the client)
            ...
            //Means select time-out there is no new message from client
				...
				//Store data in size 1000 bytes into p_out.data
 ...
 */
void readNProcessFile(char *fileName, int sockfd, packet& p_out, unsigned start_t){
	//std::string data;
	FILE *filePtr;
