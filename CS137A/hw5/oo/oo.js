...
OO.initializeCT = function() {
  ...
  var callMethod = new Method("call", 
    function(_this) { 
      ...
      var func_body = createFunctionHelper(block_stmts);
      //console.log("func_body = " + func_body);
      
      block_args = block_args.concat(func_body);
      //return _this;
      //console.log("_this = " + _this);
      //console.log("block_args = " + printArray(block_args));

      Func = Function.apply(this, block_args);
...
function createFunctionHelper(block_stmts) {
  var func_body = '';
  for(var i = 0; i < block_stmts.length ; i++) {
    /*
    if(block_stmts[i].indexOf("return")> -1) {
...
*/
    if ( i !== block_stmts.length - 1) {
      ...
      if (block_stmts[i].indexOf("return") > -1 || block_stmts[i].indexOf("var") > -1) {
        func_body += block_stmts[i];
      }
      else {
...
OO.send = function(recv, selector /* ,arg1, arg2, ... */){
  ...
  //console.log("body = " + m.m_body);
  var res =  m.m_body.apply(recv,args);
...
//keeps track of who's scope it is in for program
var THIS = null;
      ...
      */
      ...
      if (stmts.length !== 0) {
        for (var i = 0; i < stmts.length; i++) {
          var ev_stmt = this.ev(stmts[i]);
          if(ev_stmt.indexOf("return") > -1 && THIS === null){
            throw new Error("Cannot have a return outside of a method.");
          }
          else {
