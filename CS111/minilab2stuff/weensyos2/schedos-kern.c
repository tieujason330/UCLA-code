...
schedule(void)
{
	...
	case 1: //pid priority
	  while(1)
	    {
	      //run the highest priority runnable proc
	...
	case 2: //set priority
	  while(1)
	    {
	      //need highest priority num
	      pid_t x;
	      for(x = 0; x < NPROCS; x++)
		{
		  if(proc_array[x].p_priority < lowest &&
		     proc_array[x].p_state == P_RUNNABLE)
		    {
		      lowest = proc_array[x].p_priority;
		    }
		}
	      //search for first highest priority task
	      pid = (pid + 1) % NPROCS;
